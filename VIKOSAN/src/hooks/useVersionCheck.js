...
// 30 giây đếm ngược trước khi tự động cập nhật
const COUNTDOWN_SECONDS = 30;
const NOTIF_KEY = "version-update-countdown";

// Dùng chung giữa các instance để tránh 2 thông báo đếm ngược
...
const useVersionCheck = (checkInterval = 60 * 1000) => {
  ...
  const getVersionUrl = () => {
    ...
    const path = baseUrl ? `${baseUrl}/version.json` : "/version.json";
  ...
  const clearAllBrowserData = async () => {
    try {
      ...
      const paths = ["/", "/login", "/boxly", "/transfer"];
      const domains = [
  ...
  const forceReload = async () => {
    notification.destroy(NOTIF_KEY);
    globalCountdownActive = false;
    const newVersion = newVersionInfo || (await getCurrentVersion());
    if (newVersion) {
      await clearAllBrowserData();
      localStorage.setItem("app_version", JSON.stringify(newVersion));
    }
