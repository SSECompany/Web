      ...
      if (actualSttRec) {
        try {
          ...
          if (result.success && result.master && Object.keys(result.master).length > 0) {
            ...
            const maKhoXuat = masterData.ma_kho?.trim() || masterData.ma_khox?.trim() || "";
            const tenKhoXuat = masterData.ten_kho?.trim() || masterData.ten_kho_x?.trim() || "";
            const maKhoNhap = masterData.ma_khon?.trim() || masterData.ma_khonhap?.trim() || "";
            const tenKhoNhap = masterData.ten_khon?.trim() || masterData.ten_kho_nhap?.trim() || "";
            ...
            apiHandlers={{
              ...
              fetchViTriList: (keyword, record, page, type) => {
                ...
                if (!maKho) {
                  ...
                  else maKho = formValues.maKhoXuat || formValues.maKhoNhap;
                }
