...
const VatTuNhapHangTable = ({
  ...
  // Sync dataSourceRef mỗi khi dataSource thay đổi (kể cả khi cùng reference)
  useEffect(() => {
  ...
  const fetchLoList = async (keyword = "", record = {}, page = 1) => {
    try {
      ...
      const options = data.map((x) => {
        ...
        return { value, label, ngay_hhsd: x?.ngay_hhsd || null };
      });
  ...
  const handleModalOk = async () => {
    ...
    if (!newLotForm.hsd) {
      notification.warning({ message: "Vui lòng nhập hạn sử dụng (HSD)" });
      return;
    }
    const tenLo = maLo;
    ...
    try {
      ...
      const item = dataSourceRef.current.find((it) => it.key === selectedRecord?.key);
      const priceNt = parseFloat(item?.gia_nt || 0);
      const price = parseFloat(item?.gia || priceNt || 0);
      const taxRate = parseFloat(item?.thue_suat || 0);
      const soLuongGoc = (item?.dvt?.trim() === item?.dvt_goc?.trim())
        ? Math.round((soLuong / (item?.he_so_goc ?? 1)) * 1000) / 1000
        : soLuong;
      const amountNt = Math.round((priceNt * soLuong) * 100) / 100;
      const amount = Math.round((price * soLuong) * 100) / 100;
      const taxNt = Math.round((amountNt * taxRate / 100) * 100) / 100;
      const tax = Math.round((amount * taxRate / 100) * 100) / 100;

      // Update ref TRƯỚC với object MỚI (spread) để useEffect nhận ra thay đổi
      ...
      if (updatedRecord) {
        fetchLoList("", updatedRecord, 1)
          .then((result) => {
            const loOptions = Array.isArray(result) ? result : (result?.options || []);
            if (loOptions?.length > 0) {
              setDataSource((prev) =>
                prev.map((it) =>
                  it.maHang === updatedRecord.maHang
                    ? { ...it, loOptions, _loTotalPage: result?.totalPage || 1 }
                    : it
                )
              );
            }
          })
          .catch((e) => console.error("fetchLoList after lot creation error", e));
      }
  ...
  return (
    <>
      ...
      >
        ...
        <div style={{ marginBottom: 16 }}>
          <div style={{ marginBottom: 8, fontWeight: 500 }}>Hạn sử dụng <span style={{ color: 'red' }}>*</span></div>
          <DatePicker
