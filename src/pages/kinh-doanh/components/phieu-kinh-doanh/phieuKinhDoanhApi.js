...
const buildR60Table = (r60, master, unitId, userId) => {
    return r60 && r60.length > 0 ? r60.map((item, index) => ({
...
const buildMasterData = (master, unitId, userId, isUpdate = false) => {
    const data = {
        stt_rec: master.stt_rec || "",
    ma_dvcs: unitId,
    ma_ct: "DXA",
    ...
    ds_voucher: master.the_voucher || master.ds_voucher || "",
    ...
    return isUpdate ? { ...master, ...data } : data;
};

const buildDetail64 = (detail, master) => {
...
export const createPhieuKinhDoanh = async (master, detail, r60, unitId = "TAPMED", storeId = "", userId = getCurrentUserId()) => {
    // Map Master Data
    const masterData = buildMasterData(master, unitId, userId);

    // Map Detail Data
    const detail64 = buildDetail64(detail, master);

    // Map r60 Data (Chi phí) - cho cả tạo mới và sửa
    const r60Table = buildR60Table(r60, master, unitId, userId);
...
export const updatePhieuKinhDoanh = async (master, detail, r60, unitId = "TAPMED", storeId = "", userId = getCurrentUserId()) => {
    // Map Master Data
    const masterData = buildMasterData(master, unitId, userId, true);
