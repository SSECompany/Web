...
const RetailOrderInfo = ({ orderKey, currentTabOrder, ref }) => {
  ...
  const [tabOrder, setTabOrder] = useLocalStorage(
    "tabOrder"
  );
  ...
  useEffect(() => {
    const tempTabOrder =localStorage.getItem("tabOrder")?.toString()
    if (tempTabOrder == ('"'+orderKey?.toString()+'"')){
      setTimeout(()=>{
        handleShowCustomerViewDialog();
      }, 300)
      return;
  ...
  useEffect(() => {
    console.log('111',autoCalPromotion);
    ...
    switch (cellName) {
      ...
      case "so_luong":
        if (autoCalPromotion) {
          ...
          // Xử lý khi số lượng về 0 hoặc không hợp lệ
          if (!cellValue || cellValue === 0) {
            // Xóa các dòng khuyến mãi liên quan
            const updatedData = data.filter(item => !item.ck_yn || item.id !== changedRowKey);
            setData(updatedData);

            // Reset các trường liên quan đến khuyến mãi cho dòng hiện tại
            itemForm.setFieldsValue({
              [`${changedRowKey}_ck`]: 0,
              [`${changedRowKey}_tl_ck`]: 0,
              [`${changedRowKey}_ma_ck`]: "",
              [`${changedRowKey}_thue_nt`]: 0, // Reset VAT nếu có
            });
            ...
            // Tính lại khuyến mãi nếu số lượng hợp lệ
            recalPromotion();
          }
        }
          ...
          <div className="flex gap-2">
            ...
            <Tooltip placement="topRight" title="Toàn màn hình">
              <Button onClick={handleHideNavbar} className="default_button">
                <i
                  className={`pi pi-arrows-alt ${isHideNav ? "sub_text_color" : "gray_text_color"
                    } `}
                  style={{ fontWeight: "bold" }}
                ></i>
              </Button>
