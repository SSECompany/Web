...
import MealDetailsForm from "../meal/components/MealDetailsForm/MealDetailsForm";
import RoomSelectionForm from "../meal/components/RoomSelectionForm/RoomSelectionForm";
import {
  setListBeds,
  setListDepartment,
  setListDietCategory,
...
const MealTicketForm = () => {
  ...
  const mealDetails = useSelector(
    (state) => state.meal?.meals?.masterData || {}
  );
  const showFormDetails = useSelector((state) => state.meal?.showFormDetails);
  const showRoomSelection = useSelector(
    (state) => state.meal?.showRoomSelection
  );
  const listDepartment = useSelector(
    (state) => state.meal?.listDepartment || []
  );
  const listRoom = useSelector((state) => state.meal?.listRoom || []);
  const listBeds = useSelector((state) => state.meal?.listBeds || []);
  const currentBedList = useSelector((state) => state.meal?.listBeds || []);
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        const [dietRes, mealCodeRes, foodRes, departmentRes] =
          await Promise.all([
            multipleTablePutApi({
        ...
        dispatch(
          setListDepartment(departmentRes?.listObject?.dataLists?.phong || [])
        );
  ...
  useEffect(() => {
    if (mealDetails.roomCode && Array.isArray(listBeds)) {
      const filteredBeds = listBeds.filter(
        (b) => b.ma_phong?.trim() === mealDetails.roomCode
      );
      const isSame =
        JSON.stringify(currentBedList) === JSON.stringify(filteredBeds);
      if (!isSame) {
        dispatch(setListBeds(filteredBeds));
      }
    }
