...
export const createPhieuNhapKho = async (payload) => {
  ...
  try {
    ...
    if (response.data?.responseModel?.isSucceded === true) {
      message.success(
        response.data.responseModel.message || "Tạo phiếu thành công"
      );
      return { success: true };
...
export const updatePhieuNhapKho = async (payload) => {
  ...
  try {
    ...
    if (response.data?.responseModel?.isSucceded === true) {
      message.success(
        response.data.responseModel.message || "Cập nhật phiếu thành công"
      );
      return { success: true };
    } else if (response.data && response.data.statusCode === 200) {
      // Fallback for old response structure
...
export const deletePhieuNhapKho = async (stt_rec) => {
  ...
  try {
    ...
    if (response.data?.responseModel?.isSucceded === true) {
      message.success(
        response.data.responseModel.message || "Xóa phiếu thành công"
      );
      return { success: true };
    } else if (response.data && response.data.statusCode === 200) {
      // Fallback for old response structure
