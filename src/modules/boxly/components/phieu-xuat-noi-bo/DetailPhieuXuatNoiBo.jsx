...
const DetailPhieuXuatNoiBo = ({ isEditMode: initialEditMode = false }) => {
  ...
  const handleSubmit = useCallback(async () => {
    try {
      ...
      const result = await submitPhieu(
        "v1/web/cap-nhat-phieu-xuat-noi-bo",
        payload,
        isEditMode ? "Cập nhật thành công" : "Lưu thành công"
      );

      if (result.success) {
        navigate("/boxly/phieu-xuat-noi-bo");
      }
    } catch (error) {
      console.error("Lỗi khi cập nhật phiếu xuất nội bộ:", error);
    } finally {
      setLoading(false);
    }
  ...
  const handleEdit = useCallback(() => {
    navigate(`/boxly/phieu-xuat-noi-bo/edit/${stt_rec}`);
    setIsEditMode(true);
  }, [navigate, stt_rec]);
...
export default DetailPhieuXuatNoiBo;
