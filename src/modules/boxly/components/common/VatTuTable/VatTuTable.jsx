...
const VatTuTable = ({
  ...
  const handleQuantityChange = useCallback((value, record, field) => {
    const validatedValue = validateQuantityInput(value);
    onQuantityChange(validatedValue, record, field);
  }, [onQuantityChange]);

  // Render input số lượng
  const renderQuantityInput = useCallback((value, record, field, editable = true) => {
  ...
  const renderDvtSelect = useCallback((value, record) => {
    if (!isEditMode) {
      return value;
    }
      ...
      <Select
        ...
        onDropdownVisibleChange={async (visible) => {
          if (visible && record.maHang && apiHandlers.fetchDonViTinh) {
            ...
            try {
              const donViTinhList = await apiHandlers.fetchDonViTinh(record.maHang);
  ...
  }, [isEditMode, onDvtChange, loadingDvt, apiHandlers, dataSource, onDataSourceUpdate]);

  // Render select mã kho
  const renderMaKhoSelect = useCallback((value, record) => {
    if (!isEditMode) {
      return value;
    }
  ...
  }, [isEditMode, onSelectChange, loadingStates.maKho, apiHandlers, selectData.maKhoList]);
  ...
  const columns = useMemo(() => {
    ...
    if (columnConfig.showSoLuongDeNghi !== false) {
      baseColumns.push({
        ...
        align: "center",
        ellipsis: true,
        render: (value, record) => renderQuantityInput(
          value, 
          record, 
  ...
  const getScrollConfig = useCallback(() => {
    const columnWidths = columns.reduce((sum, col) => sum + (col.width || 100), 0);
