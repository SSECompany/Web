...
import { formatNumber } from "../../../Utils/hook/dataFormatHelper";
import {
  markBedAsSubmitted,
  removeMeal,
  setListFood,
...
const MealDetailsForm = () => {
  ...
  const currentBedIndex = useSelector((state) => state.meal?.currentBedIndex);
  const listBeds = useSelector((state) => state.meal?.listBeds);
  const bedName = listBeds[currentBedIndex];
  const listFood = useSelector((state) => state.meal?.listFood);
  const detailData = useSelector((state) => state.meal?.meals?.detailData);
  const [isPaid, setIsPaid] = useState(false);
  const [paymentMethod, setPaymentMethod] = useState("");

  const selectedDate = useSelector((state) => state.meal?.roomSelectedDate);
  const listMealCode = useSelector((state) => state.meal?.listMealCode);
  const listDietCategory = useSelector((state) => state.meal?.listDietCategory);
